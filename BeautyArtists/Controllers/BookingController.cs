...
namespace BeautyArtists.Controllers
{
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> BookService(int userServiceId)
        {
            ...
            // ✅ NEW: Client pays marked-up price (4% markup)
            decimal clientServicePrice = servicePrice * (1 + CLIENT_MARKUP_RATE);
            ...
            try
            {
                ...
                if (slot == null)
                {
                    ...
                    if (userService != null)
                    {
                        ...
                        model.Price = userService.Price;
                        model.BookingFee = BOOKING_FEE;
                        model.ClientTotal = (userService.Price * (1 + CLIENT_MARKUP_RATE)) + BOOKING_FEE; model.ArtistId = userService.ArtistId;
                        model.ArtistName = !string.IsNullOrEmpty(userService.Artist?.FirstName)
                            ? $"{userService.Artist.FirstName} {userService.Artist.LastName}".Trim()
                            : userService.Artist?.UserName ?? "Pro Artist";
                        model.ArtistProfilePicture = userService.Artist?.ArtistProfile?.ProfilePictureUrl ?? "/images/default-profile.png";
                ...
                decimal clientServicePrice = servicePrice * (1 + CLIENT_MARKUP_RATE);
                decimal clientTotal = clientServicePrice + bookingFee;
                decimal platformCommission = servicePrice * NEW_CLIENT_COMMISSION;  // 10% for new clients (adjusted later in PaymentService)
                decimal platformEarnings = (clientServicePrice - servicePrice) + bookingFee;  // Markup + Booking fee
        ...
        public async Task<IActionResult> ArtistUpdateStatus(int bookingId, Booking.BookingStatus newStatus, string artistNotes, decimal transportCost)
        {
            ...
            if (newStatus == Booking.BookingStatus.Accepted)
            {
                ...
                // ─── ✅ NEW: CALCULATE WITH NEW PRICING ───
                ...
                decimal depositAmount = (artistPayout / 2) + booking.BookingFee;
                decimal finalPayment = artistPayout / 2;
...
<div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 2px solid #f0c808; border-radius: 12px; padding: 20px; background: #0a0a0a; color: #fff;'>
    ...
    <div style='background: #1a1a1a; padding: 15px; border-radius: 8px; margin: 15px 0;'>
        ...
        <p><strong>Service Price:</strong> R {servicePrice:N2}</p>
        ...
        <p><strong>Platform Fee ({(isNewClient ? "10%" : "R15")}):</strong> R {(servicePrice - artistPayout):N2}</p>
...
<div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 2px solid #28a745; border-radius: 12px; padding: 20px; background: #0a0a0a; color: #fff;'>
    ...
    <p style='text-align: center; margin-top: 20px;'>✨ We hope to see you again soon! ✨</p>
        ...
        public async Task<IActionResult> CheckoutDeposit(int id)
        {
            ...
            // ─── ✅ NEW: CHECK IF NEW CLIENT ───
            bool isNewClient = await IsNewClient(currentUser.Id, booking.UserService.ArtistId);
            decimal artistPayout = CalculateArtistPayout(booking.ServicePrice, isNewClient);
            decimal platformFee = booking.ServicePrice - artistPayout;
            decimal clientMarkup = booking.ServicePrice * CLIENT_MARKUP_RATE;

            // ─── ✅ NEW: DEPOSIT AMOUNT ───
            var depositAmount = isLastMinute
                ? booking.TotalAmount
                : (artistPayout / 2) + 5.00m;
        ...
        public async Task<IActionResult> ProcessDeposit(int id, string paymentReference)
        {
            try
            {
                ...
                bool isNewClient = await IsNewClient(currentUser.Id, booking.UserService.ArtistId);
                decimal artistPayout = CalculateArtistPayout(booking.ServicePrice, isNewClient);
                decimal depositAmount = (artistPayout / 2) + 5.00m;
        ...
        public async Task<IActionResult> ProcessFinalPayment(int id)
        {
            try
            {
                ...
                bool isNewClient = await IsNewClient(currentUser.Id, booking.UserService.ArtistId);
                decimal artistPayout = CalculateArtistPayout(booking.ServicePrice, isNewClient);
                decimal remainingBalance = artistPayout / 2;
        ...
        {
            // Check if this customer has any completed or confirmed bookings with this artist
            ...
            return !existingBookings;  // True = new client, False = repeat client
        }
        ...
        private decimal CalculateArtistPayout(decimal artistPrice, bool isNewClient)
        {
            decimal platformFee = isNewClient
                ? artistPrice * NEW_CLIENT_COMMISSION  // 10% for new clients
                : REPEAT_CLIENT_FLAT_FEE;               // R15 for repeat clients

            // Safeguard: platform gets at least R8
