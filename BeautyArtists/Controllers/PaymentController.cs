    ...
    <p style='font-size: 12px; color: rgba(255,255,255,0.2);'>RubiOr</p>
        ...
        public async Task<IActionResult> InitiatePayment(int bookingId, string email, decimal amount)
        {
            try
            {
                ...
                // ✅ All money goes to your platform balance – no subaccount, no split
                // Platform fees will be deducted when releasing funds to artist
                var result = await _paymentService.InitializePaymentAsync(email, amount, bookingId, null, 0);
        ...
        public async Task<IActionResult> InitiateFinalPayment(int bookingId, string email, decimal amount)
        {
            try
            {
                ...
                // ✅ All money goes to your platform balance – no subaccount, no split
                var result = await _paymentService.InitializePaymentAsync(email, finalAmount, bookingId, null, 0);
